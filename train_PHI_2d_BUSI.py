...
from dataloaders.dataset_BUSI import (BaseDataSets, RandomGenerator,
...
parser.add_argument('--root_path', type=str,
                    default=r"D:\xxu\dataset\ultrasound\BUSI_ActiveLearning_Top26", help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='BUSI/UCSP_Improve_GVA_AFP', help='experiment_name')
...
parser.add_argument('--labeled_num', type=int, default=26,
                    help='labeled data')
# costs
...
class GVA2(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        active_scale = torch.clamp(torch.abs(self.scale), min=0.05, max=1.0)
...
def train(args, snapshot_path):
    ...
    fft_perturb = AdaptiveFrequencyPerturbation(
        ...
        max_delta=0.2, min_delta=0.01, rampup_steps=max_iterations
    ).cuda()
    ...
    cls_weight_ce   = torch.tensor([0.3, 0.7]).cuda()
    cls_weight_dice = torch.tensor([0.3, 0.7]).cuda()
...
if __name__ == "__main__":
    ...
    snapshot_path = "../compare/UCSP_Refined_Ordinary/{}_{}_labeled/{}".format(
